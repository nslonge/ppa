...
%The accessibility clause in \LLast then reflects the hypothesis that while $\varphi$-\emph{Agree} is not responsible for case valuation, it is sensitive to it: case valuation determines whether or not a given DP is accessible to $\varphi$-\emph{Agree} (see \ref{casedisc}), with accessibility parameterized across languages according to the \emph{Moravcsik Hierarchy} (see \Next[b]). The most restrictive languages make only those DPs with unmarked case accessible for agreement, while some languages also make DPs with dependent case accessible, and some even tolerate agreement with DPs bearing lexical case.
...
%%While this leaves unanalyzed several complications concerning the cross-linguistic behavior and distribution of expletives, which I return to in Section ?? (see also fns. \ref{transexp}, \ref{expl1}), we therefore correctly derive that Spec(TP) will always be filled in languages where T is associated with a \emph{Merge}-DP operation. 
...
%Allowing \emph{Agree} to be fallible in this sense is at odds with the classic minimalist view of case (\citealt{chomsky00}, \citeyear{chomsky01}), which posits that case and $\varphi$-features exist as co-dependent constraints on the derivation: each unvalued $\varphi$-feature set on a functional head must be matched with exactly one unvalued case feature on a DP, and vice versa, or the derivation crashes. I will therefore adopt the dependent model of case (\citealt{marantz84}; \citealt{bobaljik08}; \citealt{preminger14}). 
...
The second core hypothesis is that while $\varphi$-\emph{Agree} is not responsible for case valuation, it is sensitive to it. Specifically, case valuation determines whether or not a given DP is accessible to $\varphi$-\emph{Agree} (see \ref{casedisc}), with accessibility parameterized across languages according to the \emph{Moravcsik Hierarchy} (see \Next[b]). The most restrictive languages make only those DPs with unmarked case accessible for agreement, while some languages also make DPs with dependent case accessible, and some even tolerate agreement with DPs bearing lexical case.
\ex. \textbf{Case Accessibility}:
...
%Icelandic, which falls into the most restrictive category, offers a particularly clear demonstration of this effect. In ordinary transitive clauses, the verb agrees with the unmarked (nominative) subject. If, however, the subject bears quirky lexical case, here spelled out as dative, the verb agrees with the object, which has unmarked (nominative) case (see fn. \ref{oblique-nocase}). 
...
As with probing features, I will hold that EPP-feature discharge proceeds according to the basic ObOp logic: given an EPP feature \fm{F} on head H, if there is an XP in the current workspace bearing feature F that can be felicitously merged with H, it must be; if there is no such XP, the derivation proceeds intact, without discharging the feature. %I further assume that unlike with \emph{Agree}, where there is a variety of cross-linguistic evidence that a single DP can satisfy multiple $\varphi$-probes, a single XP cannot satisfy multiple EPP features on a given head.
...
%Of particular interest is the intermediate movement step, which is usually taken to be triggered by the non-interrogative embedded C. In the ObOp framework, we can capture this intermediate movement by positing that all C heads, both interrogative and not, host a \emph{wh}-EPP feature, \fm{\emph{wh}}. 
...
%If we assume that expletives are always potentially accessible for \emph{Merge} at any given stage of the derivation, then we correctly capture the fact that canonical EPP satisfaction is obligatory.
%At first glance, the ObOp view of EPP would seem to be too lenient: it allows EPP features to go undischarged, but in languages exhibiting canonical EPP effects, Spec(TP) must be filled. The resolution to this apparent problem, I argue, is to recognize that the derivation \emph{always} furnishes an XP capable of discharging the EPP feature. In ObOp terms, then, EPP satisfaction is obligatory because conditions always permit it to take place. 
...
Granting that \emph{wh}-movement stops at Spec($v$P), the antecedent for ellipsis must be at least as big as $v$P, to include the binder of the lowerst \emph{wh}-trace. However, $v$P also contains the trace of the subject, which is presumably generated $v$P-internally and moved to TP. It follows that $v$P is not a suitable antecedent. The smallest constituent that contains binders for all of the traces is therefore the sister to the highest copy in the A$'$-chain, so that \SC{MaxElide}, the principle that says ellipsis must target the largest elidable sub-constituent of the antecedent, dictates that only TP ellipsis (sluicing) is possible.\\ 
\ex. [\ \ \ \ \ \ \ \ \ \   \Tikzmark{end}{\phantom{X}}\hspace*{-1.2cm}something [1 [$_\text{TP}$\ \ \ \ \Tikzmark{end'}{\phantom{X}}\hspace*{-.6cm}John [2 \ldots $\underbrace{\text{[$_\text{$v$P}$ $t_1$ [3 [$_\text{$v$P}$ $t_2$ [$_\text{VP}$ ate $t_3$ ]]]]}}_\text{\xmark\ antecedent: $t_1$, $t_2$ free}$\hspace*{-3.9cm}\Tikzmark{int}{\phantom{X}}\hspace*{1.1cm}\Tikzmark{str'}{\phantom{X}}\hspace*{1.25cm}\Tikzmark{str}{\phantom{X}}\ \ \ \ \ \ ]]]]
\DrawArrow{str}{int}{above}{ }[-1.2]
\DrawArrow{int}{end}{above}{ }[-1.2]
\DrawArrow{str'}{end'}{above}{ }[-1.6]
%\b. [something [1 [$_\text{TP}$ John [2 \ldots [$_\text{$v$P}$ $t_1$ $\underbrace{\text{[3 [$_\text{$v$P}$ $t_2$ [$_\text{VP}$ ate $t_3$]]]}}_\text{\xmark\ antecedent: $t_2$ free}$]]]]]

In this context, Wu argues that if expletives are merged above Spec($v$P), it should be possible to take $v$P as the antecedent for ellipsis in cases where a VP-internal DP undergoes A$'$-movement. VP-ellipsis should therefore be allowed in cases parallel to \LLast involving an expletive subject.
%\ex. [\ \ \ \ \ \ \ \ \ \   \Tikzmark{end}{\phantom{X}}\hspace*{-1.2cm}several men [1 [$_\text{TP}$\ \ \ \ \Tikzmark{end'}{\phantom{X}}\hspace*{-.6cm}there [\ldots $\underbrace{\text{[$_\text{$v$P}$ $t_1$ [3 [$_\text{$v$P}$ $t_2$ [$_\text{VP}$ ate $t_3$ ]]]]}}_\text{\xmark\ antecedent: $t_1$, $t_2$ free}$\hspace*{-3.9cm}\Tikzmark{int}{\phantom{X}}\hspace*{1.1cm}\Tikzmark{str'}{\phantom{X}}\hspace*{1.25cm}\Tikzmark{str}{\phantom{X}}\ \ \ \ \ \ ]]]]
%\DrawArrow{str}{int}{above}{ }[-1.2]
%\DrawArrow{int}{end}{above}{ }[-1.2]
%\DrawArrow{str'}{end'}{above}{ }[-1.6]
This is not borne out: VP ellipsis is impossible here, as expected if expletives are merged low. 
...
I will therefore assume that \ref{low-merge} is on the right track, and adopt it here. 
\end{comment}
...
%A final comment is in order before moving on. Specifically, I will encode the fact that transitive $v$ must introduce a novel argument, i.e., that it discharges a $\theta$-role, directly in its Canonical-EPP feature. We can therefore assume that transitive $v$ has a slightly more specified feature structure, whereby the discharge of \fm{D} is directly linked to $\theta$-assignment. There are several ways of formally encoding this, with the choice among alternatives not particularly important here. For concreteness, I assume that DPs come specified with particular $\theta$-features in the lexicon, and that the EPP feature directly selects for the appropriate one. I will largely set this complication aside, although I briefly return to it in Section ??.  

The final issue that will be relevant to the proposal is the case/agreement properties of expletives. We will be concerned in the remainder of the paper with two types of expletives: (i) the grammaticalization of the default 3rd person pronoun (\emph{it} in English, \emph{il} in French, \emph{det} in MSc, etc.); (ii) the grammaticalization of the distal locative proform (\emph{there} in English, \emph{der} in MSc, etc.). In the case of the default 3.\SC{sg} expletive, hereafter DefExp, I will adopt the widespread view that the expletive differs from the semantically contentful variety only in that it is semantically vacuous. Crucially, it maintains the formal syntactic  properties of its non-expletive counterpart, and is therefore capable both of discharging probe features and triggering dependent case on a lower DP. 
%In both cases, I will adopt the minimal assumption that the expletive use of the relevant proform differs from the non-expletive use only in being semantically vacuous. Crucially, I will assume that both expletives maintain all the formal syntactic properties of their non-expletive counterparts. In the case of expletive \emph{it}, this means that the expletive interacts for case and agreement like any other pronoun, and is capable both of discharging probe features and triggering dependent case on a lower DP.
...
%\ex. \a. Step 1: \emph{Agree}(H,DP,$\varphi$), discharge [$\varphi$:\_] 
%\b. Step 2: \emph{Merge}(H,DP), discharge \fm{D} \hfill (by FM)\\\\
%\ [$_\text{HP}$ \Tikzmark{g'}{DP} [$_\text{HP}$ \Tikzmark{p}{H} [\ldots[\ldots \Tikzmark{g}{DP} \ldots]]]]
